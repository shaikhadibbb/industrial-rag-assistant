...
class DocumentChunker:
    ...
    def chunk_documents(self, documents: List[Document]) -> List[TextNode]:
        ...
        try:
            ...
            logger.info(f"Chunked {len(documents)} docs into {len(nodes)} nodes using SentenceWindow.")
        except Exception as e:
            logger.warning(f"SentenceWindowNodeParser failed, falling back to TokenTextSplitter: {e}")
            nodes = self.fallback_parser.get_nodes_from_documents(documents)
            logger.info(f"Chunked {len(documents)} docs into {len(nodes)} nodes using fallback.")
